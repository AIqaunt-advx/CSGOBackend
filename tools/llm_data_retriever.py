...
class DataRetriever:
    ...
    def get_latest_hours_data(self, hours: int = 7) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"获取时间范围: {hours_ago.strftime('%Y-%m-%d %H:%M:%S')} 到 {now.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def get_price_range_data(self, min_price: float = None, max_price: float = None, limit: int = 10) -> List[Dict[str, Any]]:
        """获取指定价格范围的数据"""
        try:
            query = {}
...
def print_beautiful_data(data: List[Dict[str, Any]], title: str = "数据样本"):
    ...
    # 打印表头
    print(f"{'序号':<4} {'时间':<20} {'价格':<8} {'在售量':<8} {'求购价':<8} {'求购量':<8} {'交易额':<10} {'交易数':<8} {'存活数':<8}")
    print("-" * 80)
    ...
    for i, record in enumerate(data, 1):
        ...
        print(f"{i:<4} {timestamp_str:<20} {price:<8.2f} {on_sale_qty:<8} {seek_price:<8.2f} {seek_qty:<8} {trans_amount:<10.2f} {trans_num:<8} {survive_num:<8}")
