    ...
    def _run_crawler_loop(self):
        ...
        try:
            ...
            self.loop.run_until_complete(
                csgo_crawler.start_scheduled_crawling())
...
def main():
    ...
    crawler_parser.add_argument('action', choices=['start', 'stop', 'status', 'restart', 'run'],
                                help='爬虫操作')
    ...
    data_parser.add_argument('--limit', '-l', type=int,
                             default=10, help='获取数量')
    data_parser.add_argument(
        '--format', '-f', choices=['json', 'pretty'], help='输出格式')
    ...
    server_parser.add_argument(
        '--with-crawler', action='store_true', help='同时启动爬虫')
